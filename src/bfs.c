...
/*   Updated: 2020/11/14 16:46:45 by archid-          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void set_walk_edges(t_queue *paths, t_queue **apath,
                           t_qnode **walk_edge) {
    t_qnode *walk;
    size_t curr;
...
bool detect_collition(t_graph *g, t_qnode *e1, t_qnode *e2, size_t curr,
                      t_qnode **walk_edge) {
...
bool discover_collition(t_graph *g, t_qnode *e1, t_qnode *e2, size_t curr,
                        t_qnode **walk_edge, t_queue **apath, int *residual) {
    t_qnode *after1;
    t_qnode *after2;
    ...
    */
    ...
    if (e2 && AS_EDGE(walk_edge[curr])->residual == AS_EDGE(e2))
        ...
        *residual = 0;
        walk_edge[curr + 1] = after2;
        move_edge = true;
    } else if (after1 &&
               AS_EDGE(after1)->residual == AS_EDGE(walk_edge[curr + 1])) {
        *residual = 1;
        walk_edge[curr] = after1;
        move_edge = true;
    } else
        *residual = 0;
    return move_edge;
...
void fix_collition(t_graph *g, t_qnode *e1, t_qnode *e2, size_t curr,
                   t_qnode **walk_edge, t_queue **apath) {
    int residual;
    bool move_edge =
        discover_collition(g, e1, e2, curr, walk_edge, apath, &residual);
    ...
    queue_node_del_next(apath[curr + residual], walk_edge[curr + residual],
                        queue_node_del_dry);
    ...
    queue_swap_halfs(apath[curr + !residual], apath[curr + residual],
                     walk_edge[curr + !residual], walk_edge[curr + residual]);
...
t_queue *re_wire_paths(t_graph *g, t_queue *paths) {
    ...
    while (true) {
        ...
        while (curr < n_paths - 1) {
            ...
            if (detect_collition(g, e1, e2, curr, walk_edge)) {
                fix_collition(g, e1, e2, curr, walk_edge, apath);
                collision = true;
                break;
            }
            curr++;
        ...
        if (collision) {
            set_walk_edges(paths, apath,
                           walk_edge); /* start from the sink again */
            continue;
        }
