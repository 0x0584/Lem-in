...
int	vertices_draw(t_graph *g, SDL_Rect dstr)
{
	SDL_Texture	*tex;
	t_vertex		*curr;
	int			*ranges_x;
	...
	// ft_printf("nbr of vertices : %zu --- max_cords x : %d - y : %d\n", g->n_vertices, g->max_c.x, g->max_c.y);
	...
	while (curr)
	{
		if (ft_strequ(curr->name, g->source->name))
			tex = get_imagetex(g->data, "resources/source.png");
		...
		// edges_draw(g->data, g, curr); // edges drawn on top of vertices
...
{
	SDL_Rect	vertices_size;

	vertices_size.w = 30;
	vertices_size.h = 30;
	vertices_draw(g, vertices_size);
	return (1);
}
#endif
